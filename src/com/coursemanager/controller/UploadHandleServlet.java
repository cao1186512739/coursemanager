...
public class UploadHandleServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
    			...
    			FileList filelist = (FileList) session2.getAttribute("filelist");
    			String cname = filelist.getC_name();
    			String cclass = filelist.getC_class();
    			String type = filelist.getC_type();
    			FileService service = new FileService();
    			List<Course> courseId = null;
    			int flag = 0;
    			try {
    				courseId = service.FindCid(cname,tid,cclass);
...
//                String savePath = this.getServletContext().getRealPath("/WEB-INF/upload");
                ...
                try{
                    ...
                    for(FileItem item : list){
                        ...
                        if(item.isFormField()){
                            ...
                            //注意：不同的浏览器提交的文件名是不一样的，有些浏览器提交上来的文件名是带有路径的，如�?  c:\a\b\1.txt，�?�有些只是单纯的文件名，如：1.txt
                            //处理获取到的上传文件的文件名的路径部分，只保留文件名部分
                            filename = filename.substring(filename.lastIndexOf("\\")+1);
                            //得到上传文件的扩展名
                            String fileExtName = filename.substring(filename.lastIndexOf(".")+1);
                            //如果�?要限制上传的文件类型，那么可以�?�过文件的扩展名来判断上传的文件类型是否合法
                    ...
                    //request.setAttribute("message", "单个文件超出最大上传值！！！");
                   
                }catch (FileUploadBase.SizeLimitExceededException e) {
                	 flag = 3;
                    e.printStackTrace();
                   
                    //request.setAttribute("message", "上传文件的大小超出限制的最大！！！");
                ...
                session3.setAttribute("tpath", path);
